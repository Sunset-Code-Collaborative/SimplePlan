...
from src.simpleplan.models import ProjectMetadata, ProjectPlan, ProjectStep
from src.simpleplan.project_plan_io import ProjectPlanIO, ProjectPlanNotFoundError
...
class TestProjectPlanModels:
    ...
    def test_project_step_creation(self):
        ...
        step = ProjectStep(id="STEP-001", description="Test step", step_type="task")
    ...
    def test_project_plan_creation(self):
        ...
        plan = ProjectPlan(project_id="test123", name="Test Project", metadata=metadata)
    ...
    def test_project_plan_completion_percentage(self):
        ...
        assert (
            abs(plan.get_completion_percentage() - 66.67) < 0.1
    ...
    def test_dependency_validation(self):
        ...
        metadata = ProjectMetadata(initiator="test_user")
        step1 = ProjectStep(
            id="STEP-001", description="Step 1", dependencies=["STEP-999"]
        )  # Non-existent
        step2 = ProjectStep(
            id="STEP-002", description="Step 2", dependencies=["STEP-001"]
        )  # Valid
...
class TestProjectPlanIO:
    ...
    def test_save_and_load_project_plan(self):
        ...
        plan = ProjectPlan(
            project_id="test123", name="Test Project", metadata=metadata, steps=[step]
        )
    ...
    def test_mark_step_complete(self):
        ...
        self.io.create_project_plan(
            name="Test Project", initiator="test_user", path=self.test_file
        )

        # Add a step
        step_id = self.io.add_step(description="Test step", path=self.test_file)
    ...
    def test_mark_step_complete_with_dependencies(self):
        ...
        step1_id = self.io.add_step(description="First step", path=self.test_file)
        step2_id = self.io.add_step(
            description="Second step", dependencies=[step1_id], path=self.test_file
        )
    ...
    def test_get_status_summary(self):
        ...
        self.io.add_step(description="Step 2", path=self.test_file)
