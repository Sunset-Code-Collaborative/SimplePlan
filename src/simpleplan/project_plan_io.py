...
from .models import ProjectPlan, ProjectStep, ProjectMetadata
...
class ProjectPlanIO:
    ...
    def save_project_plan(self, plan: ProjectPlan, path: Optional[Path] = None) -> Path:
        ...
        try:
            ...
            if validation_errors:
                raise ProjectPlanValidationError(f"Validation errors: {', '.join(validation_errors)}")
    ...
    def load_project_plan(self, path: Optional[Path] = None) -> ProjectPlan:
        ...
        try:
            ...
            validation_errors = plan.validate_dependencies()
            if validation_errors:
                self.console.print(f"⚠️  Validation warnings: {', '.join(validation_errors)}", style="yellow")
    ...
    def mark_step_complete(self, step_id: str, path: Optional[Path] = None) -> bool:
        ...
        try:
            ...
            for step in plan.steps:
                if step.id == step_id:
                    if step.complete:
                        self.console.print(f"⚠️  Step {step_id} is already complete", style="yellow")
                        return False
                    ...
                    if missing_deps:
                        self.console.print(f"❌ Cannot complete {step_id}: dependencies not met: {', '.join(missing_deps)}", style="red")
                        return False
    ...
    def add_step(self, description: str, step_type: str = "task", 
    ...
    def list_steps(self, path: Optional[Path] = None, show_completed: bool = True) -> None:
        ...
        try:
            ...
            if next_steps:
                self.console.print(f"🎯 Next available steps: {', '.join(step.id for step in next_steps)}")
    ...
    def create_project_plan(self, name: str, description: str = "", 
                           initiator: str = "AI", 
                           path: Optional[Path] = None) -> ProjectPlan:
...
def save_project_plan(plan: Union[ProjectPlan, Dict[str, Any]], path: Optional[Path] = None) -> Path:
