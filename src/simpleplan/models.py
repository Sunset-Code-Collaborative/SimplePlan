...
class ProjectStep(BaseModel):
    ...
    assigned_to: str = Field(default="AI", description="Who is assigned to complete this step")
    step_type: str = Field(default="task", description="Type of step (task, refactor, testing, etc.)")
    metadata: Dict[str, Any] = Field(default_factory=dict, description="Additional metadata for the step")
    created_at: Optional[datetime] = Field(default_factory=datetime.now, description="When this step was created")
    completed_at: Optional[datetime] = Field(default=None, description="When this step was completed")
...
class ProjectMetadata(BaseModel):
    ...
    ai_generated: bool = Field(default=True, description="Whether this plan was generated by AI")
...
class ProjectPlan(BaseModel):
    ...
    created_at: datetime = Field(default_factory=datetime.now, description="When this project was created")
    updated_at: datetime = Field(default_factory=datetime.now, description="When this project was last updated")
    steps: List[ProjectStep] = Field(default_factory=list, description="List of steps in the project")
    ...
    def get_next_available_steps(self) -> List[ProjectStep]:
        ...
        for step in self.steps:
            if not step.complete and all(dep in completed_step_ids for dep in step.dependencies):
